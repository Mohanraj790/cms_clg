// src/pages/departments/CyberSecurity.jsx
...
const CyberSecurity = () => {
  ...
  useEffect(() => {
    const loadSections = async () => {
      try {
        const res = await api.get("/departments/cyber/sections");
        if (res.data.ok) {
    ...
    const loadData = async () => {
      try {
        ...
        const peopleRes = await api.get(
          `/departments/cyber/sections/${activeTitle.id}/people`
        );
        ...
        const pdfsRes = await api.get(
          `/departments/cyber/sections/${activeTitle.id}/files`
        );
        ...
        try {
          const sheetsRes = await api.get(
            `/departments/cyber/sections/${activeTitle.id}/all`
          );
  ...
  const handleAddTitle = async () => {
    ...
    try {
      const res = await api.post(`/departments/cyber/sections`, { title });
  ...
  const handleSaveSheet = async () => {
    ...
    try {
      const response = await api.post(
        `/departments/cyber/sections/${activeTitle.id}/excel/upload`,
        fd,
        {
          headers: {
      ...
      try {
        const sheetsRes = await api.get(
          `/departments/cyber/sections/${activeTitle.id}/excel/files`
        );
        if (sheetsRes.data.ok) {
          setSheets(sheetsRes.data.data);
        }
    ...
    try {
      try {
        const res = await api.get(
          `/departments/cyber/sections/${activeTitle.id}/excel/files/${sheet.id}`
        );
        ...
        if (res.data?.ok) {
          setSheetPreviewData({
            ...
            rows: res.data.rows || [],
            downloadUrl:
              res.data.file?.downloadUrl ||
              `${BASE_URL}/uploads/excel/${sheet.filename}`,
          });
        } else {
  ...
  const testExcelAPI = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/departments/cyber/sections/${activeTitle.id}/excel/files`
      );
          ...
          {activeTitle ? (
            ...
            >
              <Title ta="center" c={maroon} order={2} mb="md">
                Welcome to Cyber Security Department
              </Title>
  ...
  async function handleSaveFaculty() {
    ...
    try {
      ...
      if (editFaculty) {
        res = await api.patch(
          `/departments/cyber/sections/${activeTitle.id}/people/${editFaculty.id}`,
          fd,
  ...
  async function handleDeleteFaculty(id) {
    try {
      await api.delete(
        `/departments/cyber/sections/${activeTitle.id}/people/${id}`
      );
  ...
  async function handleSavePDF() {
    ...
    try {
      let res;
      if (editPDF) {
        // NOTE: copied structure from your CSE code;
        // you may want to change `id` to `editPDF.id`
        res = await api.patch(
          `/departments/cyber/sections/${activeTitle.id}/files/${id}`,
          fd,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
  ...
  async function handleDeleteSheet(id) {
    try {
      await api.delete(
        `/departments/cyber/sections/${activeTitle.id}/excel/files/${id}`
      );
  ...
  async function handleDeleteSection(sectionId) {
    try {
      await api.delete(`/departments/cyber/sections/${sectionId}`);
