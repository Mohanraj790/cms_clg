// src/pages/departments/Humanities.jsx
...
const Humanities = () => {
  ...
  useEffect(() => {
    const loadSections = async () => {
      try {
        const res = await api.get("/departments/snh/sections");
        if (res.data.ok) {
    ...
    const loadData = async () => {
      try {
        ...
        const peopleRes = await api.get(
          `/departments/snh/sections/${activeTitle.id}/people`
        );
        ...
        const pdfsRes = await api.get(
          `/departments/snh/sections/${activeTitle.id}/files`
        );
        ...
        try {
          const sheetsRes = await api.get(
            `/departments/snh/sections/${activeTitle.id}/all`
          );
  ...
  const handleAddTitle = async () => {
    ...
    try {
      const res = await api.post(`/departments/snh/sections`, { title });
  ...
  const handleSaveSheet = async () => {
    ...
    try {
      const response = await api.post(
        `/departments/snh/sections/${activeTitle.id}/excel/upload`,
        fd,
        {
          headers: {
      ...
      try {
        const sheetsRes = await api.get(
          `/departments/snh/sections/${activeTitle.id}/excel/files`
        );
        if (sheetsRes.data.ok) {
          setSheets(sheetsRes.data.data);
        }
    ...
    try {
      ...
      try {
        const res = await api.get(
          `/departments/snh/sections/${activeTitle.id}/excel/files/${sheet.id}`
        );
  ...
  const testExcelAPI = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/departments/snh/sections/${activeTitle.id}/excel/files`
      );
  ...
  async function handleSaveFaculty() {
    ...
    try {
      ...
      if (editFaculty) {
        res = await api.patch(
          `/departments/snh/sections/${activeTitle.id}/people/${editFaculty.id}`,
          fd,
  ...
  async function handleDeleteFaculty(id) {
    try {
      await api.delete(
        `/departments/snh/sections/${activeTitle.id}/people/${id}`
      );
  ...
  async function handleSavePDF() {
    ...
    try {
      let res;
      if (editPDF) {
        // NOTE: copied from your CSE pattern – `id` is undefined here.
        // In real fix, you should use editPDF.id instead of id.
        res = await api.patch(
          `/departments/snh/sections/${activeTitle.id}/files/${id}`,
          fd,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
  ...
  async function handleDeleteSheet(id) {
    try {
      await api.delete(
        `/departments/snh/sections/${activeTitle.id}/excel/files/${id}`
      );
  ...
  async function handleDeleteSection(sectionId) {
    try {
      await api.delete(`/departments/snh/sections/${sectionId}`);
