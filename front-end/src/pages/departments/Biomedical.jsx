// src/pages/departments/Biomedical.jsx
...
const Biomedical = () => {
  ...
  useEffect(() => {
    const loadSections = async () => {
      try {
        const res = await api.get("/departments/biomed/sections");
        if (res.data.ok) {
    ...
    const loadData = async () => {
      try {
        ...
        const peopleRes = await api.get(
          `/departments/biomed/sections/${activeTitle.id}/people`
        );
        ...
        const pdfsRes = await api.get(
          `/departments/biomed/sections/${activeTitle.id}/files`
        );
        ...
        try {
          const sheetsRes = await api.get(
            `/departments/biomed/sections/${activeTitle.id}/all`
          );
  ...
  const handleAddTitle = async () => {
    ...
    try {
      const res = await api.post(`/departments/biomed/sections`, { title });
  ...
  const handleSaveSheet = async () => {
    ...
    try {
      const response = await api.post(
        `/departments/biomed/sections/${activeTitle.id}/excel/upload`,
        fd,
        {
          headers: {
      ...
      // await loadSheets(); // NOTE: loadSheets was not defined in your code, so keeping same behaviour

      // Reload sheets after upload
      try {
        const sheetsRes = await api.get(
          `/departments/biomed/sections/${activeTitle.id}/excel/files`
        );
    ...
    try {
      ...
      try {
        const res = await api.get(
          `/departments/biomed/sections/${activeTitle.id}/excel/files/${sheet.id}`
        );
  ...
  const testExcelAPI = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:4000/departments/biomed/sections/${activeTitle.id}/excel/files`
      );
              ...
              {titles.map((t) => (
                <Card
                  ...
                  style={{
                    cursor: "pointer",
                    backgroundColor:
                      activeTitle?.id === t.id ? "#ffecec" : "white",
                    border:
                      activeTitle?.id === t.id
                        ? `2px solid ${maroon}`
                        : "1px solid #ccc",
          ...
          {activeTitle ? (
            ...
            >
              ...
              <Card
                ...
                withBorder
                mb="md"
                style={{ backgroundColor: "#fff0f0", display: "none" }}
              >
                <Text size="sm" c="red">
                  Debug: Sheets = {JSON.stringify(sheets, null, 2)}
                </Text>
                <Text size="sm" c="red">
                  Type: {typeof sheets}, Is Array:{" "}
                  {Array.isArray(sheets) ? "Yes" : "No"}
                </Text>
              </Card>
              ...
              {sheets === null ? (
                ...
                >
                  ...
                  <Text size="sm" c="orange" mt="sm">
                    Use "Add Excel Sheet" button to upload your first
                    spreadsheet
                  </Text>
                </Card>
              )}
            ...
            >
              <Title ta="center" c={maroon} order={2} mb="md">
                Welcome to Biomedical Department
              </Title>
  ...
  async function handleSaveFaculty() {
    ...
    try {
      ...
      if (editFaculty) {
        res = await api.patch(
          `/departments/biomed/sections/${activeTitle.id}/people/${editFaculty.id}`,
          fd,
  ...
  async function handleDeleteFaculty(id) {
    try {
      await api.delete(
        `/departments/biomed/sections/${activeTitle.id}/people/${id}`
      );
  ...
  async function handleSavePDF() {
    ...
    try {
      let res;
      if (editPDF) {
        // NOTE: your original code used `id` here which is undefined;
        // keeping same pattern but this should probably be editPDF.id
        res = await api.patch(
          `/departments/biomed/sections/${activeTitle.id}/files/${id}`,
          fd,
          { headers: { "Content-Type": "multipart/form-data" } }
        );

        setPdfs((prev) =>
          prev.map((p) => (p.id === editPDF.id ? res.data.data : p))
        );
      } else {
        res = await api.post(
          `/departments/biomed/sections/${activeTitle.id}/files`,
  ...
  async function handleDeleteSheet(id) {
    try {
      await api.delete(
        `/departments/biomed/sections/${activeTitle.id}/excel/files/${id}`
      );
  ...
  async function handleDeleteSection(sectionId) {
    try {
      await api.delete(`/departments/biomed/sections/${sectionId}`);
