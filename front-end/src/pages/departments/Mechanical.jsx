// src/pages/departments/Mechanical.jsx
...
const Mechanical = () => {
  ...
  useEffect(() => {
    const loadSections = async () => {
      try {
        const res = await api.get("/departments/mech/sections");
        if (res.data.ok) {
    ...
    const loadData = async () => {
      try {
        ...
        const peopleRes = await api.get(
          `/departments/mech/sections/${activeTitle.id}/people`
        );
        ...
        const pdfsRes = await api.get(
          `/departments/mech/sections/${activeTitle.id}/files`
        );
        ...
        try {
          const sheetsRes = await api.get(
            `/departments/mech/sections/${activeTitle.id}/all`
          );
  ...
  const handleAddTitle = async () => {
    ...
    try {
      const res = await api.post(`/departments/mech/sections`, { title });
  ...
  const handleSaveSheet = async () => {
    ...
    try {
      const response = await api.post(
        `/departments/mech/sections/${activeTitle.id}/excel/upload`,
        fd,
        {
          headers: {
      ...
      try {
        const sheetsRes = await api.get(
          `/departments/mech/sections/${activeTitle.id}/excel/files`
        );
        if (sheetsRes.data.ok) {
          setSheets(sheetsRes.data.data);
        }
    ...
    try {
      ...
      try {
        const res = await api.get(
          `/departments/mech/sections/${activeTitle.id}/excel/files/${sheet.id}`
        );
  ...
  const testExcelAPI = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/departments/mech/sections/${activeTitle.id}/excel/files`
      );
  ...
  async function handleSaveFaculty() {
    ...
    try {
      ...
      if (editFaculty) {
        res = await api.patch(
          `/departments/mech/sections/${activeTitle.id}/people/${editFaculty.id}`,
          fd,
  ...
  async function handleDeleteFaculty(id) {
    try {
      await api.delete(
        `/departments/mech/sections/${activeTitle.id}/people/${id}`
      );
  ...
  async function handleSavePDF() {
    ...
    try {
      let res;
      if (editPDF) {
        // NOTE: same as your original CSE code; `id` here is undefined.
        // You probably want editPDF.id, but keeping it ditto.
        res = await api.patch(
          `/departments/mech/sections/${activeTitle.id}/files/${id}`,
          fd,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
  ...
  async function handleDeleteSheet(id) {
    try {
      await api.delete(
        `/departments/mech/sections/${activeTitle.id}/excel/files/${id}`
      );
  ...
  async function handleDeleteSection(sectionId) {
    try {
      await api.delete(`/departments/mech/sections/${sectionId}`);
