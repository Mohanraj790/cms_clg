// src/pages/departments/Agriculture.jsx
...
const Agriculture = () => {
  ...
  useEffect(() => {
    const loadSections = async () => {
      try {
        const res = await api.get("/departments/agri/sections");
        if (res.data.ok) {
    ...
    const loadData = async () => {
      try {
        ...
        const peopleRes = await api.get(
          `/departments/agri/sections/${activeTitle.id}/people`
        );
        ...
        const pdfsRes = await api.get(
          `/departments/agri/sections/${activeTitle.id}/files`
        );
        ...
        try {
          const sheetsRes = await api.get(
            `/departments/agri/sections/${activeTitle.id}/all`
          );
  ...
  const handleAddTitle = async () => {
    ...
    try {
      const res = await api.post(`/departments/agri/sections`, { title });
  ...
  const handleSaveSheet = async () => {
    ...
    try {
      const response = await api.post(
        `/departments/agri/sections/${activeTitle.id}/excel/upload`,
        fd,
        {
          headers: {
      ...
      try {
        const sheetsRes = await api.get(
          `/departments/agri/sections/${activeTitle.id}/excel/files`
        );
    ...
    try {
      ...
      try {
        const res = await api.get(
          `/departments/agri/sections/${activeTitle.id}/excel/files/${sheet.id}`
        );
  ...
  const testExcelAPI = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:4000/departments/agri/sections/${activeTitle.id}/excel/files`
      );
              ...
              {titles.map((t) => (
                <Card
                  ...
                  style={{
                    ...
                    border: activeTitle?.id === t.id ? `2px solid ${maroon}` : "1px solid #ccc",
                    transition: 'all 0.2s',
                  }}
                >
  ...
  async function handleSaveFaculty() {
    ...
    try {
      ...
      if (editFaculty) {
        res = await api.patch(
          `/departments/agri/sections/${activeTitle.id}/people/${editFaculty.id}`,
          fd,
  ...
  async function handleDeleteFaculty(id) {
    try {
      await api.delete(
        `/departments/agri/sections/${activeTitle.id}/people/${id}`
      );
  ...
  async function handleSavePDF() {
    ...
    try {
      ...
      if (editPDF) {
        res = await api.patch(
          `/departments/agri/sections/${activeTitle.id}/files/${id}`,
          fd,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
  ...
  async function handleDeleteSheet(id) {
    try {
      await api.delete(`/departments/agri/sections/${activeTitle.id}/excel/files/${id}`);
  ...
  async function handleDeleteSection(sectionId) {
    try {
      await api.delete(`/departments/agri/sections/${sectionId}`);
